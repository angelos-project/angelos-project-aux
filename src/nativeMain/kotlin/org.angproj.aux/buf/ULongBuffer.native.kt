...
public actual class ULongBuffer actual constructor(size: Int) : AbstractBufferType<ULong>(size) {
    ...
    public actual override operator fun get(index: Int): ULong {
        if(index !in 0..<(size-7)) throw IllegalArgumentException("Out of bounds.")
        return (ptr + index)!!.reinterpret<ULongVar>().pointed.value
    }

    public actual override operator fun set(index: Int, value: ULong) {
        if(index !in 0..<(size-7)) throw IllegalArgumentException("Out of bounds.")
        (ptr + index)!!.reinterpret<ULongVar>().pointed.value = value
    }
}