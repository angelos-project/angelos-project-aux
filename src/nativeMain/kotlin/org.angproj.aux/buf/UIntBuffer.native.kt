...
public actual class UIntBuffer actual constructor(size: Int) : AbstractBufferType<UInt>(size) {
    ...
    public actual override operator fun get(index: Int): UInt {
        if(index !in 0..<(size-3)) throw IllegalArgumentException("Out of bounds.")
        return (ptr + index)!!.reinterpret<UIntVar>().pointed.value
    }

    public actual override operator fun set(index: Int, value: UInt) {
        if(index !in 0..<(size-3)) throw IllegalArgumentException("Out of bounds.")
        (ptr + index)!!.reinterpret<UIntVar>().pointed.value = value
    }
}