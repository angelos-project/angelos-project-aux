scripts = """0000..001F    ; Common # Cc  [32] <control-0000>..<control-001F>
...
public object {klass}: UtfBlock {{
    ...
    override val noUse: List<Int> by lazy {{ listOf({use}) }}
}}
"""
...
def load_scripts():
    cp = set()
    for row in scripts.splitlines(False):
        if row.find(';') != -1:
            t = row.split(';')[0]
            if t.find("..") == -1:
                cp.add(int(t, 16))
...
def copy_blocks(cp):
    for row in blocks.splitlines(False):
        r, n = row.split(';')
        b, e = r.split("..")
        real = (int(b, 16), int(e, 16))
        name = n.strip()
        klass = classify(name)
        limit = (reduce_start(cp, real), reduce_end(cp, real))
        reserved = list()
        for i in range(real[0], real[1]+1):
            if i not in cp:
                reserved.append(i)
        #print(", ".join(reserved))
        meta = "{}..{} ({}/{})".format(hex(real[0]), hex(real[1]), real[1] - real[0] - len(reserved)+1, len(reserved))
        noCtrl = len(reserved) == real[1] - real[0] + 1
        ctrl = "true" if noCtrl or name in noctrl else "false"
        rl = real if noCtrl else limit
        ...
        output = tmpl(klass, name, meta, rrl, ctrl, noUse)
        #print(meta)
        #print(klass)
        #print(rrl)
        #print(len(reserved2))
        #print(noUse)
        #print(ctrl)
        with open("src/commonMain/kotlin/org.angproj.aux/utf/blk/" + klass + ".kt", "w") as fd:
            fd.write(output)
